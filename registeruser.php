<?php 
    ...
    if(!empty($_POST)) 
    {
        ...
        if(empty($_POST['username'])) { redirect("registeruser.php?e=3&code=$code"); }
        if(empty($_POST['password'])) { redirect("registeruser.php?e=4&code=$code"); } 
        if(!filter_var($_POST['email'], FILTER_VALIDATE_EMAIL)) { redirect("registeruser.php?e=5&code=$code"); }
        
        $row = fetch("SELECT 1 FROM users WHERE username='" . $_POST['username'] . "'");        
        if($row) { redirect("registeruser.php?e=1&code=$code"); }
        
        $row = fetch("SELECT 1 FROM users WHERE email='" . $_POST['email'] . "'");
        if($row) { redirect("registeruser.php?e=2&code=$code"); }

        $id = getSummonerIdFromSummonerName(str_replace(' ', '', $_POST['username']));
        if (!summonerHasRunePage($id, $code))
        {
            redirect("registeruser.php?e=6&code=$code");
        }
        else
        {
            $summonerid = getSummonerIdFromSummonerName($_POST['username']);
        }
    ...
    <body>
        ...
        <div class="wrapper">
            <div class="content">
                ...
                ?>            
                <form class="center" action="registeruser.php" method="post"> 
